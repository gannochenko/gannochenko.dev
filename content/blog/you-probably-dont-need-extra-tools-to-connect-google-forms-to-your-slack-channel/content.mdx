...
At the beginning I thought to just plug in `Zapier` or any other tool like that.
...
`GS` is sort of like `VBA` (if you know what it is 🤪), but with `ECMAScript` syntax. I got an impression that `GS` was initially built to automate Google Sheets, but for other types of documents it also can do a trick or two.
...
Sometimes instead of rushing into a third-party business process solution, it may be preferable to utilise your engineering skills. Of course, this is always a trade-off, but if you have time, then why not dare?
