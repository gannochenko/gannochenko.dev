...
Time is not money. Time is a more precious thing. I really like automating the day-to-day routine, so after getting tired of constant mis-clicks and repeated keyboard work, decided to build a tool that saves a couple of minutes.
...
module.exports = {
    ...
    releaseBranch: 'master', // the branch a CD tool listens to
...
Of course, all of this can be done manually via GitHub web interface. In time the routine actions become automatic and basically work on a subconscious level.

And yet we are human, we are error-prone.
Being part of a team, you most certainly don't want to mess up with the development pipeline, as well as with the production deployment. So, if there is a way to automate your actions, to have some tool which can give a rap over the knuckles when needed, it could come handy indeed.

### Possible improvements

Sure thing the tool is not perfect. I still need to have way to ensure all the check are green and the PR is ready to merge before actually merge it. This is something I am gonna improve soon.

---

### Credits

* Gorgeous [GitHub RESP API](https://docs.github.com/en/rest) which made this all possible
