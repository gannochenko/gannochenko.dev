...
But that is not where I want to have this finished! So instead of creating `NodePort`-type services, let's go a bit further and setup `ingress`. 

➡️ Ingress

So ingress is another step forward. Basically, ingress tells the cluster "Hey, for this domain, path and protocol route all requests to a service A. For other domain and path - to service B" (and so on). So ingress works as a router for the incoming requests.
