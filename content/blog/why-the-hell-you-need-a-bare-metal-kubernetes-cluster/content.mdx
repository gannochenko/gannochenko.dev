...
👉 you are well-aware of Minikube, MicroK8s, etc, but always wanted to have a real multi-node cluster out there,<br />
👉 you don't want to pay for a pre-configured AKS/GKE/EKS/whatever solution just yet, or not allowed to use it,<br />
...
➡️ Allow nodes to address each other by name, so I don't hard-code IP addresses:
...
➡️ K8s demands the `swap` partition to be disabled for good:
...
➡️ Install `Kubernetes` and `kubeadm`. Yep, `kubeadm` will take care of all the boring stuff here.
...
... that means: "Congratulations 🎉🎉🎉 You and I just got our own K8s clusters up and running!" Most of the tutorials usually end here, or they maybe show how to expose an Apache container via some random port.

But that is not where I want to have this finished! So instead of creating `NodePort`-type services, let's go a bit further and setup `ingress` and `load balancer`. 

➡️ Ingress

Basically, ingress tells the cluster "Hey, for this given domain, path and protocol route all requests to a service A. For other domain and path - to service B" (and so on). So ingress works as a router for the incoming requests.

Sounds quite powerful, doesn't it?
...
Please find [the latest version of Metallb](https://metallb.universe.tf/). Also, sometimes they do backward-incompatible installation instruction updates, so if something does not work, feel free to [check their docs out](https://metallb.universe.tf/installation/)!
