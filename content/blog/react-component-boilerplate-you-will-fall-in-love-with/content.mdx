...
👉 CSS-in-JS with [Styled-components](https://styled-components.com/) or [Emotion](https://emotion.sh/) for styling. It turns boring CSS into a fully-featured programming language based on JavaScript.<br />
...
I prefer going with types. This way the paradigm «prefer composition over inheritance» becomes easily feasible through `union`, in contrast with OOP-ish inheritance via `extends` keyword. 

I also

👉 use `Type` postfix for types to explicitly indicate the fact that this is not a variable,<br />
👉 have all props of the component declared as optional, because the component should gracefully handle situations when it was called without parameters.<br />
...
👉 the logic is decoupled and separated, the UI stays pretty clean and easily perceivable by just glancing at it,<br />
👉 I prepare props for each visual node, and feed these props via object spread,<br />
👉 the hook can be maintained and tested separately.
